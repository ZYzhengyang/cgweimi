...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	...
	private async getFromDb(ps: {
		...
		// 视频过滤：只返回包含视频文件的笔记
		query.andWhere(new Brackets(qb => {
			qb.where('EXISTS (SELECT 1 FROM unnest(note.attachedFileTypes) AS ft WHERE ft LIKE :videoType)', { videoType: 'video/%' })
				.orWhere('note.attachedFileTypes && ARRAY[:...videoTypes]::text[]', { videoTypes: ['video/mp4', 'video/webm', 'video/quicktime', 'video/x-msvideo', 'video/x-matroska'] });
		}));
		...
		query.andWhere(new Brackets(qb => {
			qb.where('note.replyId IS NULL')
				.orWhere(new Brackets(qb2 => {
					qb2.where('note.replyId IS NOT NULL')
